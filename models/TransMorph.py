...
try:
    ...
    # maybe its being imported from here
...
class TransFeX(nn.Module):
    def __init__(self, config):
        '''
        Similar to TransMorph, but will take a single image as input, and return a hierarchy of feature images
        It's essentially a Trans(former) Fe(ature) X(traction) model
        '''
        super(TransFeX, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        assert all([x&(x-1) == 0 for x in self.levels]), "All numbers must be powers of 2" # checking for powers of 2
        # [4, 2, 1] -> [-3, -2, -1]
        self.consider_indices = [-1-int(np.log2(x)) for x in self.levels]
        self.transformer = SwinTransformer(patch_size=config.patch_size,
                                           in_chans=config.in_chans,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
        ...
        # self.spatial_trans = SpatialTransformer(config.img_size)
        self.avg_pool = nn.AvgPool3d(3, stride=2, padding=1)
        # add extra convolution pools to get feature images
        out_chan = config.output_channels
        self.feature_img_layer = nn.ModuleList([
            nn.Conv3d(embed_dim*4, out_chan, 1, 1, 0),
            nn.Conv3d(embed_dim*2, out_chan, 1, 1, 0),
            nn.Conv3d(embed_dim, out_chan, 1, 1, 0),
            nn.Conv3d(embed_dim//2, out_chan, 1, 1, 0),
            nn.Conv3d(config.reg_head_chan, out_chan, 1, 1, 0),
        ])
        # add decoder layers for image
        # in_channels = [embed_dim*4, embed_dim*2, embed_dim, embed_dim//2, config.reg_head_chan]
        # N = len(in_channels)
        # in_channels = [in_channels[t] for t in range(N) if t-N in self.consider_indices]
        ## To decode the features into images
        self.img_decoder_layer = nn.ModuleList([
            nn.Sequential(DecoderBlock(out_chan, out_chan), nn.Conv3d(out_chan, 1, 1, 1, 0))
            for _ in self.consider_indices])

    def decode_features(self, feature_list, indices=None):
        ''' given the feature list from the network, decode into the intensity image (as extra self supervision) '''
        img_list = []
        if indices is None:
            indices = range(len(feature_list))
        for i in indices:
            img_list.append(self.img_decoder_layer[i](feature_list[i]))
        return img_list
    ...
    def forward(self, x):
        ...
        # output features
        N = len(outs)
        outs = [self.feature_img_layer[i](outs[i]) for i in range(len(outs)) if i-N in self.consider_indices]
        return outs


CONFIGS = {
    'TransFeX': configs.get_3DTransFeX_config(),
...
if __name__ == '__main__':
    # Test TransFeX here
    config = CONFIGS['TransFeX']
    config.levels = [4, 1]
    model = TransFeX(config).cuda()
    image = torch.randn(1, 1, *config.img_size).cuda()
    import time
    a = time.time()
    outs = model(image)
    b = time.time()
